  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-4 sm:gap-5">
      ...
      <div className="experience-scroll min-h-0 space-y-4 overflow-y-auto pr-1 sm:space-y-5">
        {items.map((item) => (
          <article key={`${item.company}-${item.role}`} className="border-t border-lavender-blush/10 pt-4">
            <h3 className="font-display text-xl font-medium text-lavender-blush sm:text-2xl">{item.role}</h3>
            <p className="mt-1 text-sm italic text-lavender-blush/75">
              {item.company} · {item.location}
            </p>
            <p className="text-sm text-sage">{item.period}</p>
            <p className="mt-2 text-sm leading-relaxed text-lavender-blush/75">{item.highlights}</p>
          </article>
        ))}
      </div>
    </section>
